...
# Interactive Quiz Generator Module
    ...
    if st.button('Generate Interactive Quiz'):
        if quiz_topic:
            ...
            response = openai.Completion.create(
                engine='text-davinci-003',
                prompt=f'Generate {int(num_questions)} multiple-choice quiz questions on the topic: {quiz_topic} with the correct answer marked clearly and a brief explanation for each.',
                max_tokens=1000
            )
            ...
            for idx, q in enumerate(questions):
                if q:
                    ...
                    answer = st.radio(f'Select your answer for Q{idx+1}:', ('Option A', 'Option B', 'Option C', 'Option D'), key=f'ans_{idx}')
...
# Code Generator
    ...
    if st.button('Generate Code 💡'):
        if algo_name:
            ...
            # Save the code as a downloadable file
            with open('generated_code.py', 'w') as f:
                f.write(code)
            
            # Provide download option
            with open('generated_code.py', 'rb') as f:
                st.download_button('📥 Download Generated Code', f, file_name='generated_code.py')
